...
def rotate_box(p1,p2,p3,p4,p5,p6,p7,p8,axis='x', angle = 0):
    if axis=='x': rotation_matrix = RotX(angle)
    elif axis=='y': rotation_matrix = RotY(angle)
    elif axis=='z': rotation_matrix = RotZ(angle)
...
def animate_rotation_y(points, grados=90):
    n = 0
    while n < grados:
        ax.cla()
        setaxis(-10,10,-10,10,-10,10)
        ...
        [p1_rot, p2_rot, p3_rot, p4_rot,
         p5_rot, p6_rot, p7_rot, p8_rot] = rotate_box(*points, axis='y', angle=n)
...
# Animación de la rotación en Z
def animate_rotation_z(points, grados=90):
    n = 0
    while n < grados:
        ax.cla()
        setaxis(-10,10,-10,10,-10,10)
        ...
        [p1_rot, p2_rot, p3_rot, p4_rot,
         p5_rot, p6_rot, p7_rot, p8_rot] = rotate_box(*points, axis='z', angle=n)
...
# Secuencia completa: X → Y → Z
p_after_x = animate_rotation_x(90)
p_after_y = animate_rotation_y(p_after_x, 90)
p_after_z = animate_rotation_z(p_after_y, 90)
